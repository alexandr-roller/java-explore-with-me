...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Long> {
    @Query(" select new ru.practicum.ewm.common.dto.ViewStatsDto(eh.app, eh.uri, count(eh.ip) as hits) " +
            "  from EndpointHit eh" +
    ...
    @Query(" select new ru.practicum.ewm.common.dto.ViewStatsDto(eh.app, eh.uri, count(distinct eh.ip) as hits) " +
            "  from EndpointHit eh" +
            " where eh.timestamp between :start and :end " +
            "   and (coalesce(:uris) is null or eh.uri in (:uris)) " +
            " group by eh.app, eh.uri" +
